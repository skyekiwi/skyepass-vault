...
describe('Metadata Failure', () => {
  ...
  it('Metadata Error Failure Test', async () => {
    const { contract, deployer } = await setup()
    let badMetadataResult

    badMetadataResult = await contract.query.temporaryTest(0, { signer: deployer })
    console.log(badMetadataResult.output.toHuman())

    badMetadataResult = await contract.query.temporaryTest(1, { signer: deployer })
    console.log(badMetadataResult.output.toHuman())
  })
})
